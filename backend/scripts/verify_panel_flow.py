...
async def main():
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        login_res = await client.post(
            "/auth/login", json={"email": "panel@dev.vikas", "password": "devpass123"}
        )
        ...
        expert_res = await client.post(
            "/auth/login",
            json={"email": "panel_expert@dev.vikas", "password": "devpass123"},
        )
        ...
        print(
            f" -> Governance Academic Veto Enabled: {gov_state['academic_veto_enabled']}"
        )
        ...
        print(
            f" -> Total Reviews: {len(queue)} (Urgent: {len(urgent_reviews)}, Standard: {len(standard_reviews)})"
        )

        for r in queue[:2]:
            approve_count = sum(1 for v in r["votes"] if v["vote"] == "approve")
            tally_label = (
                f"{approve_count} of {r['required_signoffs']} needed"
                if r["track"] == "urgent"
                else f"{len(r['votes'])} of {r['required_signoffs']} voted"
            )
            print(
                f"    * [{r['track'].upper()}] {r['trade_name']} ({r['district_name']}): Tally: '{tally_label}', Status: {r['decision']}"
            )
        ...
        breakdown_res = await client.get(
            f"/panel/reviews/{review_id}/score-breakdown", headers=lead_headers
        )
        bd = breakdown_res.json()
        sb = bd["score_breakdown"]
        print(
            f" -> Trade: {bd['trade_name']} | District: {bd['district_name']} | Gap Score: {bd['gap_score']}"
        )
        print(f" -> Formula: {sb.get('formula')}")
        print(
            f" -> Component 1 (Curriculum Dissimilarity 1 - S): {sb.get('dissimilarity')} (Cosine Sim: {sb.get('similarity_score')})"
        )
        print(
            f" -> Component 2 (Logarithmic Volume Factor F_V): {sb.get('volume_factor')} [Volume: {sb.get('volume')}, Weight: 60%]"
        )
        print(
            f" -> Component 3 (Recency Decay Factor R): {sb.get('recency_decay')} [Avg Age: {sb.get('avg_age_days')} days, Weight: 40%]"
        )
        print(f" -> Combined Market Demand Factor: {sb.get('market_demand_factor')}")
        print(
            " -> Verified: Mathematical, deterministic scoring with zero black-box LLM."
        )
        ...
        vote_res = await client.post(
            f"/panel/reviews/{review_id}/vote",
        ...
        print(
            f" -> Vote recorded. New Review Decision: {voted_review['decision'].upper()}"
        )
        print(
            f" -> Total Votes: {len(voted_review['votes'])}, Required: {voted_review['required_signoffs']}"
        )
        if voted_review["decision"] == "approved":
            print(
                " -> SUCCESS: 2-of-4 urgent threshold satisfied, auto-transitioned to APPROVED!"
            )
        ...
        toggle_res = await client.post(
            "/panel/governance/toggle", headers=lead_headers, json={"enabled": True}
        )
        print(
            f" -> Academic Veto toggled to: {toggle_res.json()['academic_veto_enabled']}"
        )

        # If there's another review or standard review, test reject with academic expert
        pending_reviews = [
            r for r in queue if r["id"] != review_id and r["decision"] == "pending"
        ]
        if pending_reviews:
            veto_target = pending_reviews[0]
            print(
                f" -> Testing Veto on Review: {veto_target['trade_name']} ({veto_target['track']} track)"
            )
            expert_vote_res = await client.post(
                f"/panel/reviews/{veto_target['id']}/vote",
            ...
            print(
                f" -> Decision after Academic Expert Rejection: {veto_result['decision'].upper()}"
            )
            print(f" -> Veto Used Flag: {veto_result['veto_used']}")
            if veto_result["decision"] == "rejected" and veto_result["veto_used"]:
                print(
                    " -> SUCCESS: Academic Expert rejection triggered instant statutory VETO override!"
                )
