...
async def main():
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        print(f" -> Critical Divergence Districts: {map_data['critical_districts_count']}")
        print(f" -> Formula: {map_data['formula_definition']}")
        for d in map_data["districts"][:5]:
            print(f"    * {d['name']} ({d['state']}): AHI={d['alignment_health_score']:.1f}/100 ({d['health_category']}), "
                  f"Trades={d['active_trades_count']}, Flags={d['flagged_trades_count']}, Postings={d['total_job_volume']}")

        # Pick Pune district for drilldown
        pune = next((d for d in map_data["districts"] if d["name"] == "Pune"), map_data["districts"][0])
        pune_id = pune["id"]

        # 3. District Drilldown
        print(f"\n[Step 3] Fetching trade-by-trade analytics for {pune['name']} ({pune_id})...")
        drill_res = await client.get(f"/planner/districts/{pune_id}", headers=planner_headers)
        if drill_res.status_code != 200:
            print(f"FAILED to fetch district drilldown: {drill_res.text}")
            sys.exit(1)
        drill_data = drill_res.json()
        print(f" -> District Health: {drill_data['alignment_health_score']:.1f}/100 ({drill_data['health_category']})")
        print(f" -> Total Capacity: {drill_data['total_seats']} seats | Demand Volume: {drill_data['total_postings']} openings")
        print(f" -> Found {len(drill_data['trades'])} trade rows:")
        flagged_gap_id = None
        electrician_trade_id = None
        for t in drill_data["trades"]:
            print(f"    * [{t['nsqf_code']}] {t['trade_name']}: Demand={t['job_posting_volume']}, Seats={t['seats_available']}, "
        ...
        second_district = next((d for d in map_data["districts"] if d["id"] != pune_id), None)
        if second_district and electrician_trade_id:
            ...
            for cd in [comp["district_a"], comp["district_b"]]:
                print(f"    * {cd['district_name']}: Alignment={cd['alignment_score']:.1f}, Demand={cd['job_posting_volume']}, "
        ...
        if flagged_gap_id:
            ...
            assert ann_data["new_gap_status"] == "urgent_escalation", f"Expected urgent_escalation, got {ann_data['new_gap_status']}"
        else:
        ...
        print(f" -> [JSON] District: {plan_json['district_name']} | Generated: {plan_json['generated_at']}")
        print(f"    * Net Seat Adjustment: {plan_json['total_recommended_seat_change']} seats")
        print(f"    * Total Trainer Workshops: {plan_json['total_trainer_workshops']} sessions")
        print(f"    * Total Recommendations: {len(plan_json['recommendations'])}")
        for rec in plan_json["recommendations"][:3]:
            print(f"       - Trade: {rec['trade_name']} | Seats: {rec['recommended_seat_adjustment']:+d} | Workshops: {rec['recommended_trainer_workshops']} | Priority: {rec['equipment_investment_priority']} ({rec['investment_focus']})")
        ...
        if trainee_write_res.status_code == 403:
            print(" -> [PASS] HTTP 403 Forbidden correctly enforced on non-planner role!")
        else:
            print(f" -> [FAIL] Expected HTTP 403, received {trainee_write_res.status_code}")
            sys.exit(1)
